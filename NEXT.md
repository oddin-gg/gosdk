...
- **Connection or channel drops** → the exclusive auto-delete queue is deleted with its channel — with the whole connection, or alone on a channel-level exception — so unacked deliveries AND everything published until the consumer re-binds are lost from the broker; nothing is redelivered. At the moment of loss, before the rebind, the consumer tells the recovery manager (`OnFeedChannelLost`, with the lost session's message interest), every known producer in that session's scope is flagged down with `ConnectionDown`, and the next system alive starts a snapshot recovery from the last alive gen timestamp before the loss (an alive-only session's loss has no odds gap and flags nothing). Raising the flag before the rebind matters: an alive processed on the new channel first would advance the recovery cursor past the gap. The alive-based inactivity check alone would miss any connection drop shorter than `MaxInactivity`, and would never notice a single channel's loss while alives keep flowing on another. Consumer-visible message-id dedup is not a goal.
