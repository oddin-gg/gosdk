...
- **Connection drops** → the exclusive auto-delete queues are deleted with the connection, so unacked deliveries AND everything published until reconnect + rebind are lost from the broker; nothing is redelivered. On reconnect the client tells the recovery manager (`OnFeedReconnected`), every known producer is flagged down with `ConnectionDown`, and the next system alive starts a snapshot recovery from the last alive gen timestamp before the drop. The alive-based inactivity check alone would miss any drop shorter than `MaxInactivity`. Consumer-visible message-id dedup is not a goal.
