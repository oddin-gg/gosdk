...
the SDK looks the market up in the description cache once per configured
locale, then reads a name off the cached entry for the market and for
each outcome — and does I/O when the cache is cold (an unseen market,
variant or player). A consumer that takes its names from the catalog API
