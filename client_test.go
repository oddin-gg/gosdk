...
// wedgedConnectRT serves the boot who-am-i probe normally but BLOCKS the
// producers fetch forever (ignoring ctx), simulating a custom transport
// that never returns — the exact shape behind the pre-shutdown Close
// deadlock.
type wedgedConnectRT struct {
	started   chan struct{}
	release   chan struct{}
	startOnce sync.Once
}

func (rt *wedgedConnectRT) RoundTrip(req *http.Request) (*http.Response, error) {
	reply := func(s string) *http.Response {
	...
	switch {
	case strings.Contains(req.URL.Path, "/users/whoami"):
		return reply(`<bookmaker_details response_code="OK" expire_at="2099-01-01T00:00:00+00:00" bookmaker_id="42" virtual_host="/vhost"/>`), nil
	case strings.Contains(req.URL.Path, "/descriptions/producers"):
		rt.startOnce.Do(func() { close(rt.started) })
		<-rt.release // ignore ctx — a transport that never returns
		return reply(`<producers response_code="OK"/>`), nil
	default:
		return reply(`<response response_code="OK"/>`), nil
	}
}

// TestClient_Close_DoesNotHangOnWedgedConnect is the regression for the
// pre-shutdown deadlock (Codex P1): Close(context.Background()) pre-waited
// on the in-flight Connect's connectDone bounded ONLY by the caller ctx —
// with no deadline and a connect wedged in producerManager.Open on a
// cancellation-ignoring transport, that wait never returned, so
// runShutdown (which owns the shutdown budget and cancels the API
// lifetime) never started. Post-fix Close starts runShutdown immediately;
// runShutdown's own connectDone wait is bounded by WithShutdownTimeout, so
// Close returns with a budget-exceeded error instead of hanging.
func TestClient_Close_DoesNotHangOnWedgedConnect(t *testing.T) {
	rt := &wedgedConnectRT{started: make(chan struct{}), release: make(chan struct{})}
	var releaseOnce sync.Once
	release := func() { releaseOnce.Do(func() { close(rt.release) }) }
	defer release() // unwedge the connect on the way out (fatal paths too)

	cfg := NewConfig("tok", types.IntegrationEnvironment,
		WithAPIHost("api.example.test"),
		WithHTTPClient(&http.Client{Transport: rt, Timeout: 30 * time.Second}),
		WithShutdownTimeout(500*time.Millisecond),
	)
	bootCtx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	c, err := New(bootCtx, cfg)
	cancel()
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	...
	<-rt.started // Connect is now wedged in the producers fetch

	closeDone := make(chan error, 1)
	go func() { closeDone <- c.Close(context.Background()) }() // NO deadline

	select {
	case err := <-closeDone:
		if !errors.Is(err, context.DeadlineExceeded) {
			t.Fatalf("Close err = %v, want DeadlineExceeded (budget exceeded with connect pending)", err)
		}
	case <-time.After(5 * time.Second):
		t.Fatal("Close(context.Background()) hung on a wedged connect — pre-shutdown deadlock")
	}

	release() // let the wedged connect unwind
	<-connectDone
}
...
func TestClient_ConnectionEvents_TranslatesFeedEvents(t *testing.T) {
	...
	// onFeedEvent is gated on connectState != NotConnected (so
	// replay-only AMQP opens don't surface — see §23.2) AND
	// EventConnected is additionally gated on state == Connected
	// (§26 / F3 — feed-layer Connected during Connecting is suppressed
	// because ensureNormal's success path emits explicitly).
	// Simulate the post-Connect runtime state where natural reconnects
	// flow through.
...
// closeClientOnCleanup registers a t.Cleanup that closes c with a FRESH
// bounded context from context.Background(). Cleanups must NOT derive the
// close context from t.Context() (nor a defer-cancelled request context):
// testing.T.Context() is cancelled BEFORE cleanups run, so Client.Close
// would observe a dead context, return context.Canceled, and START but
// not JOIN background teardown — letting it overlap the next test or the
// httptest server's own Close and weakening the lifecycle/leak assertions
// these tests exist to make. A cleanup-time close failure is reported.
func closeClientOnCleanup(t *testing.T, c *Client) {
	t.Helper()
	t.Cleanup(func() {
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
		if err := c.Close(ctx); err != nil {
			t.Errorf("client close in cleanup: %v", err)
		}
	})
}
...
func TestClient_MarketDescriptions(t *testing.T) {
	...
	desc, err := c.MarketDescription(ctx, 1, types.None[string]())
	if err != nil {
...
func TestClient_ClearMethods(t *testing.T) {
	...
	c.ClearMarketDescription(1, types.None[string]())
}
