...
func (f *fakeSportsInfo) ListOfMatches(context.Context, int, int) ([]types.Match, error) {
	return nil, nil
}
func (f *fakeSportsInfo) LocalizedListOfMatches(context.Context, int, int, types.Locale) ([]types.Match, error) {
	return nil, nil
}
...
// v2.20 Multi* parity stubs — replay manager only depends on Match,
// so the rest can stay no-op.
func (f *fakeSportsInfo) MultiLocalizedActiveTournaments(context.Context, []types.Locale) ([]types.Tournament, error) {
	return nil, nil
}
func (f *fakeSportsInfo) MultiLocalizedSportActiveTournaments(context.Context, string, []types.Locale) ([]types.Tournament, error) {
	return nil, nil
}
func (f *fakeSportsInfo) MultiLocalizedMatchesFor(context.Context, time.Time, []types.Locale) ([]types.Match, error) {
	return nil, nil
}
func (f *fakeSportsInfo) MultiLocalizedLiveMatches(context.Context, []types.Locale) ([]types.Match, error) {
	return nil, nil
}
func (f *fakeSportsInfo) MultiLocalizedMatch(ctx context.Context, id types.URN, _ []types.Locale) (types.Match, error) {
	return f.Match(ctx, id)
}
func (f *fakeSportsInfo) MultiLocalizedCompetitor(context.Context, types.URN, []types.Locale) (types.Competitor, error) {
	return types.Competitor{}, nil
}
func (f *fakeSportsInfo) MultiLocalizedFixtureChanges(context.Context, []types.Locale, time.Time) ([]types.FixtureChange, error) {
	return nil, nil
}
func (f *fakeSportsInfo) MultiLocalizedListOfMatches(context.Context, int, int, []types.Locale) ([]types.Match, error) {
	return nil, nil
}
func (f *fakeSportsInfo) MultiLocalizedAvailableTournaments(context.Context, types.URN, []types.Locale) ([]types.Tournament, error) {
	return nil, nil
}
func (f *fakeSportsInfo) Tournament(context.Context, types.URN) (types.Tournament, error) {
	return types.Tournament{}, nil
}
func (f *fakeSportsInfo) LocalizedTournament(context.Context, types.URN, types.Locale) (types.Tournament, error) {
	return types.Tournament{}, nil
}
func (f *fakeSportsInfo) MultiLocalizedTournament(context.Context, types.URN, []types.Locale) (types.Tournament, error) {
	return types.Tournament{}, nil
}
func (f *fakeSportsInfo) ClearMatch(types.URN)       {}
func (f *fakeSportsInfo) ClearFixture(types.URN)     {}
func (f *fakeSportsInfo) ClearMatchStatus(types.URN) {}
func (f *fakeSportsInfo) ClearTournament(types.URN)  {}
func (f *fakeSportsInfo) ClearCompetitor(types.URN)  {}
func (f *fakeSportsInfo) ClearPlayer(types.URN)      {}
func (f *fakeSportsInfo) ClearSport(types.URN)       {}
func (f *fakeSportsInfo) Sport(context.Context, types.URN) (types.Sport, error) {
	return types.Sport{}, nil
}
func (f *fakeSportsInfo) LocalizedSport(context.Context, types.URN, types.Locale) (types.Sport, error) {
	return types.Sport{}, nil
}
func (f *fakeSportsInfo) MultiLocalizedSport(context.Context, types.URN, []types.Locale) (types.Sport, error) {
	return types.Sport{}, nil
}
func (f *fakeSportsInfo) MultiLocalizedSports(context.Context, []types.Locale) ([]types.Sport, error) {
	return nil, nil
}
func (f *fakeSportsInfo) Player(context.Context, types.URN) (types.Player, error) {
	return types.Player{}, nil
}
func (f *fakeSportsInfo) LocalizedPlayer(context.Context, types.URN, types.Locale) (types.Player, error) {
	return types.Player{}, nil
}
func (f *fakeSportsInfo) MultiLocalizedPlayer(context.Context, types.URN, []types.Locale) (types.Player, error) {
	return types.Player{}, nil
}
...
func TestReplay_Play(t *testing.T) {
	...
	if _, err := mgr.Play(t.Context(), types.ReplayPlayParams{
		Speed:             types.Some(10),
		MaxDelayInMs:      types.Some(50),
		RewriteTimestamps: types.Some(true),
	}); err != nil {
		t.Fatalf("Play: %v", err)
	}
...
func TestReplay_Status(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	cfg := &minimalCfg{}
	mgr := NewManager(newAPIClient(t, srv), cfg, &fakeSportsInfo{})
	got, err := mgr.Status(t.Context())
	if err != nil {
		t.Fatalf("Status: %v", err)
	}
	if got != "playing" {
		t.Errorf("status = %q, want %q", got, "playing")
	}
}
...
// TestReplay_ReplayEventIDs_NoSportsInfoCalls verifies the v2.19 fix
// for finding F5 (.NET parity): EventIDs returns the queued URNs
// directly from the API response without resolving them to Match
// values. .NET's IReplayManager exposes this as GetEventsInQueue;
// callers who only need "is X queued?" should not pay the
// per-event sports-info round-trip ReplayList incurs.
func TestReplay_ReplayEventIDs_NoSportsInfoCalls(t *testing.T) {
	body := `<?xml version="1.0"?>
<replay_set_content>
  <replay_event id="od:match:42" position="0"/>
  <replay_event id="od:match:99" position="1"/>
</replay_set_content>`
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/xml")
		_, _ = io.WriteString(w, body)
	...
	ids, err := mgr.ReplayEventIDs(t.Context())
	if err != nil {
		t.Fatalf("ReplayEventIDs: %v", err)
	}
	if len(ids) != 2 {
		t.Fatalf("got %d ids, want 2", len(ids))
	}
	if ids[0].ToString() != "od:match:42" || ids[1].ToString() != "od:match:99" {
		t.Errorf("ids = %v / %v", ids[0], ids[1])
	}
	if got := si.calls.Load(); got != 0 {
		t.Errorf("sportsInfoManager.Match calls = %d, want 0 (no resolution)", got)
	}
}
