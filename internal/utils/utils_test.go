...
// TestTimestamp_UnmarshalPreservesMilliseconds verifies the v2.19 fix
// to finding F2: _unmarshalUnixTime now uses time.UnixMilli to keep
// millisecond precision. The previous time.Unix(sec/1000, 0) form
// integer-divided away milliseconds, so XML feed timestamps with
// non-zero ms (which Java/.NET expose at full ms resolution) parsed
// to the wrong moment when Go consumers compared timestamps across
// SDKs.
func TestTimestamp_UnmarshalPreservesMilliseconds(t *testing.T) {
	var ts Timestamp
	// 2025-01-15 00:00:00.123 UTC.
	if err := ts.UnmarshalText([]byte("1736899200123")); err != nil {
		t.Fatalf("Unmarshal: %v", err)
	}
	got := time.Time(ts).UTC()
	if got.Nanosecond() != 123_000_000 {
		t.Errorf("Nanosecond = %d, want 123_000_000 (123ms preserved)", got.Nanosecond())
	}
}
